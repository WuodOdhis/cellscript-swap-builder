...
pub fn build_swap(input: &SwapInput) -> Result<SwapOutput> {
    ...
    // The generated CellScript entry wrapper reads raw cellc entry-witness bytes
    // from the current script group's witness surface by default.
    ...
    let ckb_tx = serde_json::json!({
        ...
        "witnesses": [
            format!("0x{}", hex::encode(&entry_witness)),
        ],
    });
    ...
    Ok(SwapOutput {
        ckb_tx,
        tx_hash,
        witness_hex: format!("0x{}", hex::encode(&entry_witness)),
